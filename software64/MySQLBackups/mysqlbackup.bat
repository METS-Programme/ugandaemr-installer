...
set errorLogPath="C:\Users\Lubwama\Desktop\MySQLBackups\backupfiles\dumperrors.txt"

:: MySQL EXE Path
set mysqldumpexe="C:\Program Files\MySQL\MySQL Server 5.6\bin\mysqldump.exe"

:: Error log path
set backupfldr=C:\Users\Lubwama\Desktop\MySQLBackups\backupfiles\

:: Path to data folder which may differ from install dir
set datafldr="C:\Program Files\MySQL\MySQL Server 5.6\data"

:: Path to zip executable
set zipper="C:\Users\Lubwama\Desktop\MySQLBackups\zip\7za.exe"
...
%mysqldumpexe%  openmrs --user=%dbuser% --password=%dbpass%> "%backupfldr%openmrs.%backuptime%.sql"

echo "Zipping all files ending in .sql in the folder"


:: .zip option clean but not as compressed
%zipper% a -tzip "%backupfldr%openmrsbackup.%backuptime%.zip" "%backupfldr%*.sql"
