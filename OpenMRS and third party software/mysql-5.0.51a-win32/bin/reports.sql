...
-- ----------------------------
-- Function structure for getCptStartDate2
-- ----------------------------
DROP FUNCTION IF EXISTS `getCptStartDate2`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `getCptStartDate2`(`personid` integer) RETURNS date
    READS SQL DATA
BEGIN
	#Routine body goes here...

	RETURN (select obs_datetime from obs where concept_id in(99033,99037) and personid=person_id and voided=0 order by obs_datetime asc limit 1 );
END;;
DELIMITER ;
...
-- ----------------------------
-- Function structure for getEnrolDate2
-- ----------------------------
DROP FUNCTION IF EXISTS `getEnrolDate2`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `getEnrolDate2`(`personid` integer) RETURNS date
    READS SQL DATA
BEGIN
	#Routine body goes here...
...
-- ----------------------------
-- Function structure for getEnrolEncounterId
-- ----------------------------
DROP FUNCTION IF EXISTS `getEnrolEncounterId`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `getEnrolEncounterId`(`personid` integer) RETURNS int(11)
    READS SQL DATA
BEGIN
	RETURN (select encounter_id from encounter
where encounter_type=1
and patient_id=personid
and voided=0
order by encounter_datetime asc
limit 1
);
...
where concept_id in(99064,90315,99061)
and a.voided=0
and personid=person_id
group by person_id
limit 1
...
-- ----------------------------
-- Function structure for getWhoStageBaseTxt_copy
-- ----------------------------
DROP FUNCTION IF EXISTS `getWhoStageBaseTxt_copy`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `getWhoStageBaseTxt_copy`(`personid` integer, `artstartdt` date) RETURNS int(11)
    READS SQL DATA
BEGIN
#Routine body goes here...
